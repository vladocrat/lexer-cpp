 ...
 */
infix fun <T, S> OrdinaryParser<T>.seq(other: OrdinaryParser<S>): OrdinaryParser<Pair<T, S>> =
 ...
 * If at least one fails, then the whole result is the first failure.
 */
infix fun <T, S> OrdinaryParser<T>.seq(other: SkipParser<S>): OrdinaryParser<T> =
 ...
 * If at least one fails, then the whole result is the first failure.
 */
infix fun <T, S> SkipParser<T>.seq(other: OrdinaryParser<S>): OrdinaryParser<S> =
 ...
 * If at least one fails, then the whole result is the first failure.
 */
infix fun <T, S> SkipParser<T>.seq(other: SkipParser<S>): SkipParser<Pair<T, S>> =
